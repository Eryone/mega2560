...
ISR(TIMER1_COMPA_vect) {
      char tmp_d[32];
     if((digitalRead(82)==1)&&(P_file_name[0])&&(recovery==0)&&(print_job_timer.isRunning()==true))// filament out
     {
        SERIAL_ECHOLN("filament out off");
        recovery=4;
      
     }
     if((digitalRead(85)==0)&&(P_file_name[0])&&(recovery==0)&&(print_job_timer.isRunning()==true))// power off
	  {
		 SERIAL_ECHOLN("power Down");
		// enquecommand("M929");
