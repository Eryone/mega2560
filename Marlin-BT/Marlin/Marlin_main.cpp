	...
	//	return;
	 ...
	 while ((c = Serial3.read()) >= 0)
	 {
	   ...
	   if(serial_char==0)
	   	...
	   	cn=0;
	   serial_line_buffer[cn++]=c;
	    if(bt_status==1&&cn>=6)
	    {
	    	//found
			if(strstr_P(serial_line_buffer, PSTR("Select")))
			{
				Serial3.println(F("C"));
				cn=0;
		...
		else if(bt_status==2)
		{
			if(serial_char==')'&&(cn>4))
			{
				...
				for(c=0;c<cn;c++)
				{
					if(serial_line_buffer[c]=='(')
					{
						thermalManager.current_temperature[0]=atoi(serial_line_buffer+c+1);
						k=1;
					}
					else if((serial_line_buffer[c]=='/')&&(k==1))
					{
						 
						// thermalManager.setTargetHotend(atoi(serial_line_buffer+c+1), 0);
						unsigned short tar_tmp=atoi(serial_line_buffer+c+1);
						//if(tar_tmp>5)//
						//	thermalManager.target_temperature[0]= atoi(serial_line_buffer+c+1);
					  	if(thermalManager.target_temperature[0]!=tar_tmp)
						{// set target temperature
							char tmp_f[10];
							sprintf(tmp_f,"t%d;",thermalManager.target_temperature[0]);
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 954: gcode_M954();    break;  // M105: Report Temperatures (and say "ok")
